...
static SQL_METHODS_PATTERN: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"(?i)\b(query|sql_query|execute|raw_sql)\b").unwrap());

static WEAK_HASH_PATTERN: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"\b(md5|sha1|Md5|Sha1|MD5|SHA1)\b").unwrap());
    ...
    fn check(&self, source: &str, tree: &tree_sitter::Tree) -> Vec<Finding> {
        ...
        walk_tree(tree.root_node(), source, &mut |node, src| {
            if node.kind() == "call_expression" {
                if let Some(func) = node.child_by_field_name("function") {
                    let func_text = &src[func.byte_range()];
                    if SQL_METHODS_PATTERN.is_match(func_text) {
                        if let Some(args) = node.child_by_field_name("arguments") {
    ...
    fn check(&self, source: &str, tree: &tree_sitter::Tree) -> Vec<Finding> {
        ...
        walk_tree(tree.root_node(), source, &mut |node, src| {
            ...
            if node.kind() == "use_declaration" {
                ...
                if WEAK_HASH_PATTERN.is_match(text) {
                    let algo =
            ...
            if node.kind() == "call_expression" {
                if let Some(func) = node.child_by_field_name("function") {
                    let func_text = &src[func.byte_range()];
                    if WEAK_HASH_PATTERN.is_match(func_text) {
